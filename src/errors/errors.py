...
class BaseCobraError:
    ...
    def __repr__(self):
        ...
        return (self.generate_traceback()
                ...
                + "\n    " + "-" * self.start_position.column + colorama.Fore.RED
                + "~" * (self.end_position.column - self.start_position.column)
                + colorama.Style.RESET_ALL + "-" * (len(line) - self.end_position.column - 1)
                + f"\ncobralang > {colorama.Fore.RED}ERROR{colorama.Style.RESET_ALL}: {self.error_type}\n"
...
class IllegalCharError:
    ...
    def __repr__(self):
        ...
        return (f"cobralang > {colorama.Fore.RED}ERROR{colorama.Style.RESET_ALL}: Illegal Character '{self.char}'\n"
                ...
                + "\n    " + "-" * self.position.column + "^" + (len(line) - self.position.column - 2) * "-")
...
class ExpectedCharError:
    ...
    def __repr__(self):
        ...
        line = self.position.ftxt[idx_start:idx_end]
        return (f"cobralang > {colorama.Fore.RED}ERROR{colorama.Style.RESET_ALL}: {self.details}\n"
                f'  File "{self.position.filename}", line {self.position.line + 1}'
                + "\n    "
                + line[1:self.position.column + 1]
                + colorama.Fore.YELLOW
...
class UnexpectedEOFError:
    ...
    def __repr__(self):
        ...
        return (f"cobralang > {colorama.Fore.RED}ERROR{colorama.Style.RESET_ALL}: Unexpected EOF while parsing string\n"
                + self.details
...
class InvalidSyntaxError:
    ...
    def __repr__(self):
        ...
        line = self.start_position.ftxt[idx_start:idx_end][1:]
        return (f"cobralang > {colorama.Fore.RED}ERROR{colorama.Style.RESET_ALL}: Invalid Syntax\n"
                f'{self.error_message}\n'
                ...
                + "\n    " + "-" * self.start_position.column + colorama.Fore.RED
                + "~" * (self.end_position.column - self.start_position.column)
                + colorama.Style.RESET_ALL + "-" * (len(line) - self.end_position.column))


class RuntimeError(BaseCobraError):
...
class NotImplementedError(BaseCobraError):
    def __init__(self, start_position, end_position, error_message, context):
