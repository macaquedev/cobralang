...
class CobraFunction(BaseFunction):
    ...
    def copy(self):
        return CobraFunction(self.name, self.body_node, self.arg_names, self.should_auto_return).set_context(
...
class Interpreter:
    ...
    def visit_func_def_node(node, context):
        ...
        func_value = CobraFunction(func_name, body_node, arg_names, node.should_auto_return).set_context(context).set_pos(
            node.start_position, node.end_position)
