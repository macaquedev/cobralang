...
class BaseToken:
    ...
    def __repr__(self):
        if self.value:
            try:
                return f"[{colorama.Fore.YELLOW}{self.__class__.__name__}({self.value}){colorama.Style.RESET_ALL} at {colorama.Fore.GREEN}[{self.start_position.line + 1}:{self.start_position.column + 1}]{colorama.Style.RESET_ALL}]"
            except AttributeError:
                return f"[{colorama.Fore.YELLOW}{self.__class__.__name__}({self.value}){colorama.Style.RESET_ALL} at {colorama.Fore.GREEN}[UNKNOWN]{colorama.Style.RESET_ALL}]"
        else:
            try:
                return f"[{colorama.Fore.YELLOW}{self.__class__.__name__}{colorama.Style.RESET_ALL} at {colorama.Fore.GREEN}[{self.start_position.line + 1}:{self.start_position.column + 1}]{colorama.Style.RESET_ALL}]"
            except AttributeError:
                return f"[{colorama.Fore.YELLOW}{self.__class__.__name__}{colorama.Style.RESET_ALL} at {colorama.Fore.GREEN}[UNKNOWN]{colorama.Style.RESET_ALL}]"
